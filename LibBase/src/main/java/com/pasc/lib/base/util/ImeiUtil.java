...
class ImeiUtil {
    ...
    public static ArrayList getImeiList(Context context) {
        ...
        if (Build.VERSION.SDK_INT >= 22) {
            ...
            ArrayList<String> deviceIdList = mothodFromPhone(context, "getDeviceIdGemini");
            if (deviceIdList == null || deviceIdList.size() == 0) {
                deviceIdList = mothodFromPhone(context, "getDefault");
            }
            if (deviceIdList == null || deviceIdList.size() == 0) {
                deviceIdList = mothodFromPhoneMsim(context);
            }
            if (deviceIdList == null || deviceIdList.size() == 0) {
                deviceIdList = mothodFromPhoneFactory(context);
            }
    ...
    private static boolean notSame(String value) {
    ...
    private static boolean isValidImei(String imei) {
        ...
        return length > 10 && length < 20 && notSame(imei);
    }
     ...
     * @return
     */
    private static ArrayList mothodFromPhone(Context context, String methodName) {
        try {
            TelephonyManager telephonyManager = (TelephonyManager) context.getSystemService(Context.TELEPHONY_SERVICE);
     ...
     * @return
     */
    private static ArrayList mothodFromPhoneFactory(Context context) {
        try {
            TelephonyManager telephonyManager = (TelephonyManager) context.getSystemService(Context.TELEPHONY_SERVICE);
     ...
     * @return
     */
    private static ArrayList mothodFromPhoneMsim(Context context) {
        try {
